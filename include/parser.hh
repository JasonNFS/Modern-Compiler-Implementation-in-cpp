...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    TOK_NUMBER = 46,               // "number"
    TOK_STRINGV = 47,              // "stringv"
    TOK_NEG = 48                   // NEG
      };
...
switch (yykind)
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_STRINGV (std::string v, location_type l)
      {
        return symbol_type (token::TOK_STRINGV, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_STRINGV (const std::string& v, const location_type& l)
      {
        return symbol_type (token::TOK_STRINGV, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
